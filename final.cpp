...
void cover() //Name of Project
{
		...
		for(int n=0;n<122;n++)
	{
		...
		cout<<endl;
	cout<<"\n\n\n\n\n\n\n\t\t\tCopyrights 2019, DIGITAL Dictionary, All Rights Reserved.";
	for(int n=0;n<28;n++)
	{
		cout<<".";
		Sleep(350);
...
Node* search_trie( Node* current,char* word){

	while(*word!='\0'){
		if(current->children_[*word-'a']!=NULL){
			current=current->children_[*word-'a'];
			++word;
		}
		else
			break;
...
void InsertNode(Node* trieTree,char* word1){
	Node* currentNode = trieTree;
	while(*word1!='\0'){
		if(currentNode->children_[*word1-CASE] == NULL){
			currentNode->children_[*word1-CASE]=new Node();
			currentNode->children_[*word1-CASE]->parent_=currentNode;	
		}
		currentNode=currentNode->children_[*word1-CASE];
		++word1;
	...
	system("cls");						
		...
		cout<<"\n\n_______________________________________________________________________________________________________________________";
		cout<<"\t\tCopyrights 2019, DIGITAL Dictionary, All Rights Reserved\t  Press ENTER to continue";
		getch();
	     
	     //goto menue;
};

void InsertNode1(Node* trieTree,char* word1){
	Node* currentNode = trieTree;
	while(*word1!='\0'){
		if(currentNode->children_[*word1-CASE] == NULL){
			currentNode->children_[*word1-CASE]=new Node();
	...
	++currentNode->occurrences_;
//	cout<<"Word is successfully entered in the dictionary\n";
};


void InsertNode2(Node* trieTree,string word, string syn, string ant){
	...
	currentNode->synonyms = syn;
	currentNode->antonyms = ant;
	
	++currentNode->occurrences_;
};


Node* Search(Node* trieTree,char* abc)
{
	while(*abc!='\0')
	{
		if (trieTree->children_[*abc - CASE]!=NULL){
			trieTree=trieTree->children_[*abc - CASE];
			++abc;
		}
		else return NULL;
	}
	...
	Node* main_Search(Node* trieTree,char* abc)
{
	while(*abc!='\0')
	{
		if (trieTree->children_[*abc - CASE]!=NULL){
	...
	//return (trieTree->occurrences_!=0) ? trieTree : NULL;
...
void meaning(Node* trieTree,char* abc){
	int x=1;
		//	ifstream file("history.txt",ios::out);
	Node* currentNode=Search(trieTree,abc);
	if(currentNode!=NULL){
		
		


		char *abc2[4803][2]={	{"a","(ay) the first letter of the English alphabet."},
...
{"zia-ahmed"," Zi.."}
		...
		for(main=0;main<4803;main++)
		{
			for(int n=0;n<1;n++)
			{
				...
				{
					if(abc2[main][n][k]==(NULL))
					{
						k=length;
					}
					else if(abc[k]==abc2[main][n][k])
					{
						m++;
					}
					else
...
////synonyms//////
void Synonyms(Node* trieTree,char* abc){
	int x=1;
	
	Node* currentNode=Search(trieTree,abc);
	
	if(currentNode!=NULL){
		//cout<<"Synonyms of the word are:\n";		
		//cout<<currentNode->synonyms;
		system("cls");						
		cout<<"\n\n\n\n\n\n\n\n\n\n\n_______________________________________________________________________________________________________________________";
	    cout<<"\n\n\n\t\t\t\t\t\t\tRESULT";
	    cout<<"\n\t\t\t\t\t\t\t______\n\n\n\n\n";
...
void autocomplete( Node* trieTree, vector <char> word,char* prefix){
	
	bool nochild=true;
	int X=1;
	int x=1;
if(x=1){// if word.size() is not kept here , it gonna print the prefix so keep this check
...
for(int i=0;i<ALPHABET_SIZE;i++){
	if(trieTree->children_[i]!=NULL){
       // nochild=false;// it has a child
		word.push_back(i+'a');
		autocomplete(trieTree->children_[i],word,prefix);
		word.pop_back();
	}
}
word.pop_back();
...
////Antonyms//////
void Antonyms(Node* trieTree,char* abc){
	int x=1;
...
//		cout<<endl<<endl;
//		cout<<trieTree->synonyms<<endl;
//		cout<<trieTree->antonyms<<endl;
...
int main(){
	...
	InsertNode2(trieTree,"tumid","swollen, distended, protuberant, inflated, turgid, bombastic, pompous, high-flown, stilted, grandiloquent","smooth, flat, level, plane, equable, uninflated, subdued");
InsertNode2(trieTree,"tumult","uproar, ferment, disturbance, turbulence, mutiny, insubordination, excitement, outbreak, fray, bustle, distraction, turmoil, disorder, confusion, noise, bluster, brawl, riot","peace, pacification, subsidence, quiet, tranquillity, order, orderliness");
InsertNode2(trieTree,"turbid","foul, thick, muddy, impure, unsettled, disordered, roiled","clear, limpid, crystalline");
InsertNode2(trieTree,"tutor","guardian, governor, instructor, teacher, preceptor, professor, master, savant","ward, pupil, scholar, student, disciple, learner, tyro");
InsertNode2(trieTree,"twine","twist, wind, embrace, entwine, wreath, bind, unite, braid, bend, meander","untwist, unwind, separate, disunite, detach, unwreath, unravel, disentwine, continue, straighten");
InsertNode2(trieTree,"twist","contort, convolve, complicate, pervert, distort, wrest, wreath, wind, encircle, form, weave, insinuate, unite, interpenetrate","straighten, untwist, rectify, verify, represent, reflect, render, preserve, express, substantiate, unwreath, unwind, detach, disengage, separate, disunite, disentangle, disincorporation, unravel,");
InsertNode2(trieTree,"type","mark, stamp, emblem, kind, character, sign, symbol, pattern, archetype, form, model, idea, image, likeness, expression, cast, mold, fashion","non-description, non-classification, inexpression, misrepresentation, misindication, falsification, abnormity, deviation, caricature, monstrosity");
InsertNode2(trieTree,"typify","prefigure, adumbrate, predelineate, prerepresent, foreshow, predemonstrate, foreshadow","verify, fulfill, realize");
InsertNode2(trieTree,"ugly","loathsome, hideous, hateful, frightful, uncouth, ill-favored, unsightly, ill-looking, plain, homely, deformed, monstrous, ungainly","attractive, fair, seemly, shapely, beautiful, handsome");
InsertNode2(trieTree,"umpire","judge, referee, arbiter, arbitrator","litigant, disputant");
InsertNode2(trieTree,"union","junction, coalition, combination, agreement, harmony, conjunction, concert, league, connection, alliance, confederacy, concord, confederation, consolidation","disjunction, separation, severance, divorce, disagreement, discord, disharmony, secession, disruption, multiplication, diversification, division");
InsertNode2(trieTree,"unison","harmony, agreement, concord, union","discord, disharmony, disagreement, disunion, variance");
InsertNode2(trieTree,"unit","ace, item, part, individual","total, aggregate, collection, sum, mass");
InsertNode2(trieTree,"unite","join, combine, link, attach, amalgamate, associate, coalesce, embody, merge, be_mixed, conjoin, connect, couple, add, incorporate, with, cohere, concatenate, integrate, converge","disjoin, sever, dissociate, separate, disamalgamate, resolve, disconnect, disintegrate, disunite, disrupt, divide, multiply, part, sunder, diverge");
InsertNode2(trieTree,"unity","oneness, singleness, individuality, concord, conjunction, agreement, uniformity, indivisibility","plurality, multitude, complexity, multiplicity, discord, disjunction, separation, severance, variety, heterogeneity, diversity, incongruity, divisibility");
InsertNode2(trieTree,"uphold","support, sustain, elevate, raise, countenance, advocate, sanction, encourage, defend, maintain, vindicate, stand_by","subvert, drop, discard, betray, discountenance, oppose, discourage, attack, repudiate, abandon, abjure, renounce");
InsertNode2(trieTree,"uproot","eradicate, extirpate, weed, deracinate, exterminate, destroy","implant, sow, foster, nurture, cultivate");
InsertNode2(trieTree,"upset","overturn, overthrow, capsize, overbalance, subvert, disestablish","establish, plant, stabilitate, corroborate, confirm");
InsertNode2(trieTree,"urbane","affable, courteous, refined, polite","boorish, discourteous, fined, coarse");
InsertNode2(trieTree,"urgent","pressing, imperative, immediate, importunate, forcible, strenuous, serious, grave, momentous, indeferrible","unimportant, insignificant, trifling, trivial, deferrable");
InsertNode2(trieTree,"useful","advantageous, profitable, helpful, serviceable, beneficial, available, adapted, suited, conducive","disadvantageous, unprofitable, obstructive, retardative, preventative, antagonistic, hostile, cumbersome, burdensome, unbeneficial, unavailable, inconducive, useless, fruitless, ineffectual");
InsertNode2(trieTree,"usher","herald, introduce, precede, announce","follow, attend, succeed");
InsertNode2(trieTree,"usual","common, customary, ordinary, normal, regular, habitual, wonted, accustomed, general","uncommon, rare, exceptional, uncustomary, extraordinary, abnormal, irregular, unusual");
InsertNode2(trieTree,"usurp","seize, arrogate, appropriate, assume","receive, inherit, accept");
InsertNode2(trieTree,"vacant","empty, leisure, unemployed, unencumbered, unoccupied, void, unfilled, mindless, exhausted","full, replenished, business, employed, engaged, occupied, filled, thoughtful");
InsertNode2(trieTree,"vacate","resign, surrender, annul, invalidate, abrogate, neutralize","hold, retain, fill, occupy, substantiate");
InsertNode2(trieTree,"vagary","whim, fancy, crotchet, freak, caprice","purpose, determination, judgment, conviction, seriousness");
InsertNode2(trieTree,"vague","general, lax, indefinite, undetermined, popular, intangible, equivocal, unsettled, uncertain, ill-defined, pointless","strict, definite, determined, limited, scientific, pointed, specific");
InsertNode2(trieTree,"vain","empty, worthless, fruitless, unsatisfying, unavailing, idle, ineffectual, egotistic, showy, unreal, conceited, arrogant","solid, substantial, sound, worthy, efficient, effectual, cogent, potent, unconceited, modest, real");
InsertNode2(trieTree,"valid","strong, powerful, cogent, weighty, sound, substantial, available, efficient, sufficient, operative, conclusive","weak, invalid, powerless, unsound, unsubstantial, unavailable, inefficient, insufficient, inoperative, obsolete, effete, superseded, inconclusive");
InsertNode2(trieTree,"vanish","disappear, dissolve, melt, fade_away","appear, approach, loom");
InsertNode2(trieTree,"vanity","emptiness, unsubstantiality, unreality, falsity, conceit, self-sufficiency, ostentation, pride, worthlessness, triviality","substance, solidity, substantiality, reality, truth, modesty, selfdistrust, simplicity, unostentatiousness, humility");
InsertNode2(trieTree,"vary","alter, dissimilate, change, modify, variegate, diversify, differ, deviate, disagree","perpetuate, stabilitate, assimilate, stereotype, harmonize, conform");
InsertNode2(trieTree,"vast","waste, wild, desolate, extensive, spacious, wide_spread, gigantic, wide, boundless, measureless, enormous, mighty, huge, immense, colossal, prodigious, far-reaching","narrow, close, confined, frequented, populated, cultivated, tended, tilled, limited, bounded, circumscribed, moderate");
InsertNode2(trieTree,"vere","shift, tergiversate, trim, temporize","stand, stick, persist, remain, adhere");
InsertNode2(trieTree,"veil","intercept, hide, screen, disguise, palliate, mask, cover, conceal","expose, unveil, strip, denude, exaggerate");
InsertNode2(trieTree,"venial","pardonable, excusable, remissible, justifiable","unpardonable, inexcusable, irremissible, mortal");
InsertNode2(trieTree,"verbal","oral, spoken, parole, vocal, unwritten, unrecorded","written, documentary, epistolary, recorded");
InsertNode2(trieTree,"verify","establish, confirm, fulfill, authenticate, substantiate, identify, realize, test, warrant, demonstrate","disestablish, subvert, fail, falsify, mistake, misrepresent, misstate, disappoint, misdemonstrate");
InsertNode2(trieTree,"verity","truth, reality, actuality, in_existence","falsity, unreality, supposition, hypothesis, conjecture, chimera, dream, delusion, fancy, phantasy");
InsertNode2(trieTree,"vernal","spring, youthful, genial, balmy, nascent","wintry, aged, brumal, hibernal, harsh, ungenial, cutting, autumnal, decadent, declining");
InsertNode2(trieTree,"vest","kobe, clothe, furnish, endow","divest, strip, unrobe, denude, deprive");
InsertNode2(trieTree,"vex","tease, irritate, provoke, plague, torment, tantalize, bother, worry, pester, trouble, disquiet, afflict, harass, annoy","soothe, appease, gratify, quiet, please");
InsertNode2(trieTree,"vice","corruption, fault, defect, evil, crime, immorality, sin, badness","purity, faultlessness, perfection, virtue, immaculateness, goodness, soundness");
InsertNode2(trieTree,"victim","sufferer, prey, sacrifice, martyr, dupe, grill","sacrificer, seducer");
InsertNode2(trieTree,"view","behold, examine, inspect, explore, survey, consider, contemplate, reconnoitre, observe, regard, estimate, judge","ignore, overlook, disregard, misconsider, misinspect, misobserve, misestimate, misjudge");
InsertNode2(trieTree,"view","sight, vision, survey, examination, inspection, judgment, estimate, scene, representation, apprehension, sentiment, conception, opinion, object, aim, intention, purpose, design, end, light, aspect","blindness, occultation, obscuration, darkness, misexamination, deception, error, delusion, misjudgment, misrepresentation, misconception, airlessness, nonintention");
InsertNode2(trieTree,"vile","cheap, worthier, valueless, low, base, mean, despicable, hateful, bad, impure, vicious, abandoned, abject, sinful, sordid, ignoble, wicked, villainous, degraded, wretched","costly, rare, precious, valuable, high, exulted, noble, honorable, lofty, venerable");
InsertNode2(trieTree,"vilify","debase, degrade, spoil, deteriorate, mar, defame, traduce, asperse, stigmatize, slander, upbraid, abuse, decry, cheapen","purify, refine, exalt, raise, improve, enhance, eulogize, praise, laud, panegyrize, commend");
InsertNode2(trieTree,"virile","manly, masculine, robust, nervous, vigorous","feeble, debile, puerile, effeminate, emasculate");
InsertNode2(trieTree,"virtue","power, capacity, strength, force, efficacy, excellence, value, morality, goodness, uprightness, purity, chastity, salubrity","weakness, incapacity, inability, inefficacy, badness, corruption, vice, immorality, impurity, unchastity, virulence, malignancy");
InsertNode2(trieTree,"viscid","glutinous, ropy, gelatinous, sticky, cohesive","watery, limpid, tenacious, incohesive");
InsertNode2(trieTree,"vital","living, animate, life-supporting, life-containing, essential, necessary, important, indispensable, inseparable, paramount, material","mortal, lethal, cadaverous, lifeless, inanimate, unessential, unimportant, secondary, separable, immaterial");
InsertNode2(trieTree,"vogue","way, custom, fashion, repute, use, usage, practice","desuetude, disuse, disesteem, unfashionableness, disrepute, abolition");
InsertNode2(trieTree,"voice","tone, utterance, language, articulation, words, expression, signification, opinion, vote, suffrage, say, control","aphony, muteness, obmutescence, voicelessness, silence, inarticulation, inexpression, dumbness, inauthoritativeness");
InsertNode2(trieTree,"void","wanting, empty, vacant, useless, nugatory, destitute, bereft, unoccupied, unfilled, unsubstantial, lacking, invalid, null","possessed, endued, furnished, full, occupied, solid, substantial, rich, valid, operative, good, efficacious");
InsertNode2(trieTree,"volume","size, body, bulk, dimensions, book, work, tome, capacity, magnitude, compass, quantity","diminutiveness, tenuity, minuteness, smallness");
InsertNode2(trieTree,"vouch","attest, undertake, promise, assure, warrant, vow","demur, repudiate, decline, abnegate, abjure, renounce, protest");
InsertNode2(trieTree,"vulgar","popular, general, loose, ordinary, public, vernacular, plebeian, uncultivated, unrefined, low, mean, coarse, underbred","strict, scientific, philosophical, restricted, technical, accurate, patrician, select, choice, cultivated, refined, polite, high-bred, stylish, aristocratic");
InsertNode2(trieTree,"wag","joker, jester, droll_fellow, humorist","sober_fellow, serious_fellow");
InsertNode2(trieTree,"wages","remuneration, hire, compensation, stipend, salary, allowance","gratuity, douceur, premium, bonus, grace");
InsertNode2(trieTree,"walt","stay, tarry, rest, stop, bide, expect","speed, hasten, press");
InsertNode2(trieTree,"wake","rouse, suscitate, excite, reanimate, revive, awake, watch, revel, evoke, summon, provoke, call","soothe, allay, appease, lay, hush, lullaby, mesmerize, tranquilize, quiet");
InsertNode2(trieTree,"walk","step, stride, march, stalk, plod, tread, tramp, trudge","halt, stop, stand_still, ride");
InsertNode2(trieTree,"wider","ramble, range, stroll, rove, expatiate, roam, deviate, stray, depart, err, swerve, straggle, saunter, navigate, circumnavigate, travel","rest, stop, perch, bivouac, halt, lie, anchor, alight, settle, moor, pause, repose");
InsertNode2(trieTree,"wane","fade, pale, decrease, diminish, decline, fail, sink, ebb, deteriorate, recede, pine, droop, attenuate, contract","brighten, increase, improve, mend, advance, rally, develop, recover, expand");
InsertNode2(trieTree,"want","deficiency, lack, failure, insufficiency, scantiness, shortness, omission, neglect, nonproduction, absence","supply, sufficiency, provision, abundance, production, allowance, supplement, adequacy");
InsertNode2(trieTree,"wanton","wandering, roving, sportive, playful, frolicsome, loose, unbridled, uncurbed, reckless, unrestrained, irregular, licentious, dissolute, inconsiderate, heedless, gratuitous","stationary, unrovlng, unsportive, unplayful, unfrolicsome, joyless, thoughtful, demure, sedate, discreet, staid, self-controlled, well-regulated, formal, austere, purposed, deliberate, cold-blooded, determined");
InsertNode2(trieTree,"ward","watch, avert, defend, parry, guard, fend, repel","betray, surrender, admit");
InsertNode2(trieTree,"warm","blood-warm, thermal, genial, irascible, hot, ardent, affectionate, fervid, fervent, fiery, glowing, enthusiastic, zealous, eager, excited, interested, animated","frigid, cold, tepid, starved, indifferent, cool, unexcited, passionless");
InsertNode2(trieTree,"warmth","ardor, glow, fervor, zeal, heat, excitement, intensity, earnestness, cordiality, animation, eagerness, vehemence, geniality, sincerity, passion, irascibility, emotion, life","frigidity, frost, congelation, iciness, coldness, calmness, coolness, indifference, torpidity, insensitiveness, apathy, slowness, ungeniality, insincerity, passionlessness, hypocrisy, good-temper, death");
InsertNode2(trieTree,"warn","premonish, monish, admonish, notify, dissuade, deter, alarm","encourage, incite, instigate, induce");
InsertNode2(trieTree,"warp","turn, twist, shrink, give, contort, bias, unhinge, distort, prejudice, corrupt, narrow, pervert","stand, hold, direct, guide, control, correct, regulate, train, conduct, rectify, expand");
InsertNode2(trieTree,"wary","suspicious, cautious, watchful, guarded, circumspect, prudent, vigilant","unwary, unsuspecting, incautious, invigilant, unwatchful, unguarded, uncircumspect, heedless");
InsertNode2(trieTree,"wash","lave, cleanse, bathe, rinse, absterge","soil, foul, contaminate");
InsertNode2(trieTree,"washy","weak, vapid, diluted, spiritless, pointless","vigorous, nervous, forcible, pregnant, trenchant, pungent");
InsertNode2(trieTree,"waste","ruin, destroy, devastate, impair, consume, squander, dissipate, throw_away, diminish, impair, lavish, desolate, pine, decay, attenuate, dwindle, shrivel, wither, wane","restore, repair, conserve, preserve, perpetuate, protect, husband, economize, utilize, hoard, treasure, accumulate, enrich, flourish, luxuriate, multiply, augment, develop");
InsertNode2(trieTree,"watch","wait, wake, contemplate, observe, note, tend, guard","disregard, overlook, misobserve");
InsertNode2(trieTree,"waver","hesitate, dubitate, halt, fluctuate, vacillate, alternate, scruple, be_undetermined, totter","determine, decide, rest, repose, settle");
InsertNode2(trieTree,"weak","feeble, infirm, enfeebled, powerless, debile, fragile, incompact, inadhesive, pliant, frail, oft, tender, milk_and_water, flabby, flimsy, wishy_washy, destructible, watery, diluted, imbecile, inefficient, spiritless, foolish, injudicious, unsound, undecided, unconfirmed, irrepressible, wavering, ductile, easy, malleable, unconvincing, inconclusive, vapid, pointless","strong, vigorous, robust, muscular, nervous, powerful, tough, stout, lusty, sturdy, compact, adhesive, resistant, fibrous, hard, indistructible, potent, intoxicating, efficient, spirited, animated, wise, sound, judicious, cogent, valid, decided, determined, unwavering, stubborn, unyielding, inexorable, conclusive, irresistible, forcible, telling");
InsertNode2(trieTree,"weaken","debilitate, enfeeble, enervate, dilute, impair, paralyze, attenuate, sap","strengthen, invigorate, empower, corroborate, confirm");
InsertNode2(trieTree,"wealth","influence, riches, mammon, lucre, plenty, affluence, abundance, opulence","indigence, poverty, scarcity, impecuniosity");
InsertNode2(trieTree,"weapon","instrument, implement, utensil, arm","shift, makeshift, contrivance");
InsertNode2(trieTree,"wear","carry, bear, exhibit, sport, consume, dou, waste, impair, rub, channel, groove, excavate, hollow, diminish","doff, abandon, repair, renovate, renew, increase, swell, augment");
InsertNode2(trieTree,"weazen","wizened, withered, dried_up","fresh, plump, fair, rounded_out, full");
InsertNode2(trieTree,"wed","link, marry, espouse","separate, divorce");
InsertNode2(trieTree,"weight","gravity, ponderosity, heaviness, pressure, burden, importance, power, influence, efficacy, consequence, moment, impressiveness","lightness, levity, portableness, alleviation, unimportance, insignificance, weakness, inefficacy, unimpressiveness, triviality, worthlessness");
InsertNode2(trieTree,"whole","total, entire, all, well, complete, sound, healthy, perfect, unimpaired, undiminished, integral, undivided, gross","partial, imperfect, incomplete, unsound, sick, impaired, diminished, fractional, divided, sectional");
InsertNode2(trieTree,"wicked","evil, bad, godless, sinful, immoral, iniquitous, criminal, unjust, unrighteous, irreligious, profane, ungodly, vicious, atrocious, black, dark, foul, unhallowed, nefarious, naughty, heinous, flagitious, abandoned, corrupt","good, virtuous, just, godly, moral, religious, upright, honest, pure, honorable, incorrupt, sinless, spotless, immaculate, stainless");
InsertNode2(trieTree,"wield","manage, handle, employ, sway, brandish","mismanage, misemploy, discard, resign, depose, surrender, deposit, abdicate");
InsertNode2(trieTree,"wife","consort, spouse, helpmeet, helpmate","mistress, husband");
InsertNode2(trieTree,"wild","untamed, undomesticated, uncultivated, uninhabited, desert, savage, uncivilized, unrefined, rude, ferocious, untrained, violent, ferine, loose, disorderly, turbulent, ungoverned, inordinate, disorderly, chimerical, visionary, incoherent, raving, distracted, haggard","tame, domesticated, cultivated, inhabited, frequented, populous, civilized, polite, refined, reclaimed, gentle, mild, subdued, regulated, orderly, rational, collected, coherent, sane, sober, sensible, calm, trim");
InsertNode2(trieTree,"wile","guile, craft, cunning, artifice, art, device, machination, plot, design, stratagem","openness, candor, ingenuousness, friendliness, frankness, artlessness");
InsertNode2(trieTree,"will","allure, procure, gain, obtain, conciliate, earn, succeed, get, achieve, accomplish, conquer","repel, forfeit, miss, alienate, fail");
InsertNode2(trieTree,"winnow","sift, simplify, eliminate, disencumber, sort","confuse, amalgamate, confound, intermix");
InsertNode2(trieTree,"wisdom","knowledge, erudition, learning, enlightenment, attainment, information, discernment, judgment, sagacity, prudence, light","ignorance, illiterateness, sciolism, indiscernment, injudiciousness, folly, imprudence, darkness, empiricism, smattering, inacquaintance");
InsertNode2(trieTree,"wit","mind, intellect, sense, reason, understanding, humor, ingenuity, imagination","mindlessness, senselessness, irrationality, dulness, stolidity, stupidity, inanity, doltishness, wash, vapidity, platitude, commonplace");
InsertNode2(trieTree,"wither","shrivel, dry, collapse, shrink, blast, blight","swell, grow, freshen, luxuriate, wanton, refresh, expand, develop");
InsertNode2(trieTree,"witty","jocose, humorous, facetious, acute","stupid, sober, dull, stolid");
InsertNode2(trieTree,"woe","grief, sorrow, misery, calamity, affliction, distress, disaster, trouble, malediction, curse","joy, gladness, comfort, boon, happiness, prosperity, weal, benediction, blessing");
InsertNode2(trieTree,"wonder","amazement, astonishment, surprise, admiration, phenomenon, prodigy, portent, miracle, sign, marvel","astonishment, indifference, apathy, unamazement, anticipation, expectation, familiarity, triviality");
InsertNode2(trieTree,"wont","rule, custom, use, habit","exception, deviation, disuse, desuetude");
InsertNode2(trieTree,"wood","thicket, grove, forest, cope","open_place");
InsertNode2(trieTree,"word","term, expression, message, account, tidings, order, vocable, signal, engagement, promise","idea, conception");
InsertNode2(trieTree,"work","exertion, effort, toil, labor, employment, performance, production, product, effect, result, composition, achievement, operation, is_sue, fruit","effortlessness, inertia, rest, inoperativeness, non-employment, nonperformance, non-production, abortion, miscarriage, frustration, neutralization, fruitlessness");
InsertNode2(trieTree,"worry","harass, irritate, tantalize, importune, vex, molest, annoy, tease, torment, disquiet, plague, fret","soothe, calm, gratify, please, amuse, quiet");
InsertNode2(trieTree,"wound","rend, cut, hurt, injure, harm, damage, pain, mortify, annoy, gall, irritate, lacerate","heal, soothe, allay, repair, mollify, soften, gratify, please");
InsertNode2(trieTree,"wrap","wind, fold, muffle, cover, involve, infold, envelop, encumber","unwind, unfold, develop, unwrap, eliminate");
InsertNode2(trieTree,"wrath","ire, passion, rage, fury, anger, exasperation, indignation, resentment","gratification, approval, delight, appeasement, pacification, reconciliation, compassion, leniency, mercy");
InsertNode2(trieTree,"wrong","unfit, unsuitable, improper, mistaken, incorrect, erroneous, unjust, illegal, inequitable, immoral, injurious, awry","fit, suitable, proper, correct, accurate, right, just, legal, equitable, fair, moral, beneficial, straight");
InsertNode2(trieTree,"wry","atwist, askew, contorted, distorted, deformed, deranged","straight, right, just, fit, proper, shapely, comely");
InsertNode2(trieTree,"yield","furnish, produce, afford, bear, render, relinquish, give_in, let_go, forego, accede, acquiesce, resign, surrender, concede, allow, grant, submit, succumb, comply, consent, agree","withdraw, withhold, retain, deny, refuse, vindicate, assert, claim, disallow, appropriate, resist, dissent, protest, recalcitrate, struggle, strive");
InsertNode2(trieTree,"zealot","partisan, bigot, enthusiast, fanatic, devotee, visionary","renegade, traitor, deserter");
InsertNode2(trieTree,"zenith","height, highest_point, pinnacle, acme, summit, culmination, maximum","nadir, lowest_point, depth, minimum");
InsertNode2(trieTree,"zephyr","west_wind, mild_breeze, gentle_wind","gale, furious_wind");
InsertNode2(trieTree,"zero","naught, cipher, nothing","something, an_existence, creature");
InsertNode2(trieTree,"zest","flavor, appetizer, gusto, gust, pleasure, enjoyment, relish, sharpener, recommendation, enhancement","distaste, disrelish, detriment");
	...
	int choice, flag=1, value;
		...
		cout<<"\n\t\t1 Enter the new word in the dictionary :"<<endl;
		cout<<"\t\t2 Search the meaning of word in the dictionary :"<<endl;
		cout<<"\t\t3 Search Word from the dictionary :"<<endl;
		cout<<"\t\t4 Search the synonyms of word in the dictionary :"<<endl;
		cout<<"\t\t5 Search the antonyms of word in the dictionary :"<<endl;
		cout<<"\t\t6 History of last searches is:"<<endl;
		cout<<"\t\t7 Print all the words of the dictionary :"<<endl;
	...
	cout<<"\n\t\t\t\t\t  Enter a word to insert:\n\t\t\t\t\t\t\t";
			...
			InsertNode(trieTree,word1);
			goto menue;
  // meaning(trieTree,abc);
			break;
	...
	cout<<"\n\t\t\t\t\t  Enter a word to search:\n\t\t\t\t\t\t\t\t\t";
		...
		goto menue;
           		...
           		break;
           		case 7:
           		//cout<<"All words that exists in the dictionary are :";
           		search();
	...
	cout<<"\n\t\t\t\  Enter a word to search:\n\t\t\t\t\t\t";
           		...
           		Node* temp=search_trie(trieTree,abc);
           		if(temp==NULL)
           			{
					   cout<<"NO word with this prefix in dictionary "<<endl;
           				goto menue;}
           		else
           			autocomplete(temp,ch,abc);
